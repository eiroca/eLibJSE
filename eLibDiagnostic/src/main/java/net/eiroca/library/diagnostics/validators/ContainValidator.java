...
public class ContainValidator extends Validator {
  ...
  public void setup(final IContext context) throws CommandException {
    super.setup(context);
    searchString = context.getConfigString(ContainValidator.CONFIG_SEARCH_STRING, "");
  }
  ...
  public boolean isValid(final String content) throws CommandException {
    ...
    if (!verified && !silent) {
      CommandException.Invalid(MessageFormat.format("Expected string \"{0}\" didn't match.", searchString));
    }
    return verified;
  }
